...
function ArticleFeed({ articles, currentUser }: any) {
  const [articlesDisplayCount, setArticlesDisplayCount] =
    useState(articleDisplayCount);
  const [hasMore, setHasMore] = useState(true);
  const [addedArticles, setAddedArticles] = useState(
    articles.slice(0, articleDisplayCount)
  );
  const fetchMoreData = () => {
    ...
    setTimeout(() => {
      articles
        .slice(articlesDisplayCount, articlesDisplayCount + articleDisplayCount)
    ...
    <>
      {addedArticles.length > 0 ? (
        <InfiniteScroll
          ...
          hasMore={hasMore}
          loader={
            <p className="flex items-center justify-center h-[10vh]">
          ...
          endMessage={
            <p style={{ textAlign: "center" }} className="mt-3 pb-0 p-2">
              <b>
                Uncover Hidden Gems Beyond This Point - More awaits in
